    ...
    ReverseClaimer
{
    ...
    function canModifyName(
        ...
        (address _owner, uint32 fuses, uint64 expiry) = getData(uint256(node));
        return
            (owner() == addr || _owner == addr || isApprovedForAll(_owner, addr)) &&
    ...
    function _wrapETH2LD(
        ...
        bytes memory name = _addLabel(label, "\x03test\x00");
        names[node] = name;
